...
const seasonRecucer = (state = '', action) => { // stateを変化させるreducer
  switch (action.type) {
...
const allReducers = combineReducers({ //ここに並べたものから順番にreducerのifやswithを全て読んでいく
    season: seasonRecucer,
    user: userReducer,
  }
);

const store = createStore(allReducers); //store設置 reducerは必須
